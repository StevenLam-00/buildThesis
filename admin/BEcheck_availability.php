...
if (!empty($_POST["username"])) {
    ...
    $sql = "SELECT username FROM boardeditor WHERE username=:uName";
    ...
    if ($query->rowCount() > 0) {
        ...
        echo "<script>$('#addBE').prop('disabled',true);</script>";
    } else {

        echo "<span style='color:green'> User Name is available for Registration.</span>";
        echo "<script>$('#addBE').prop('disabled',false);</script>";
    }
}
...
if (!empty($_POST["email"])) {
    $email = $_POST["email"];
    $sql = "SELECT email FROM boardeditor WHERE email=:email";
    $query = $dbh->prepare($sql);
