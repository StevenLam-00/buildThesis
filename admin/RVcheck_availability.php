...
if (!empty($_POST["username"])) {
	...
	$sql = "SELECT username FROM reviewer WHERE username=:uName";
	...
	if ($query->rowCount() > 0) {
		...
		echo "<script>$('#addRV').prop('disabled',true);</script>";
	} else {

		echo "<span style='color:green'> User Name is available for Registration.</span>";
		echo "<script>$('#addRV').prop('disabled',false);</script>";
	}
}
...
if (!empty($_POST["email"])) {
	$email = $_POST["email"];
	$sql = "SELECT email FROM reviewer WHERE email=:email";
	$query = $dbh->prepare($sql);
